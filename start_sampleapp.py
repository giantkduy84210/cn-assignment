...
def index(headers, body):
    ...
    if cookies and cookies.get("auth") == "true":
        return {"body": open("www/index.html").read(), "status_code": 200, "headers": {"Content-Type": "text/html"}}
    else:
...
def submit_info(headers, body):
    ...
    try:
        ...
        return {"status_code": 400, "body": "Invalid request", "headers": {"Content-Type": "text/plain"}}

@app.route('/get-list', methods=['GET'])
def get_list(headers, body):
    """Peer yêu cầu danh sách peer đang online"""
    return {"status_code": 200, "body": json.dumps(peers), "headers": {"Content-Type": "application/json"}}
